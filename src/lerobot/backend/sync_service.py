...
# 설정
LEROBOT_OUTPUT_DIR = "/home/nsm_industry_technology_team/Desktop/NSM_Lerobot/outputs/train" # LeRobot 기본 저장 경로
...
def find_new_episodes():
    ...
    for ds_path in datasets:
        ...
        # episodes 폴더 확인 (LeRobot 버전에 따라 다를 수 있음, 보통 root에 있거나 episodes 폴더에 있음)
        # Assuming standard: {root}/{repo_id}/... we look for parquet files recursively
        
        # LeRobotDataset 저장 방식:
        # {root}/{repo_id}/data/chunk-{i}.parquet
        # {root}/{repo_id}/videos/{camera}/{episode}.mp4
       # 2. 관련 비디오 파일 찾기
    # timestamp 매칭이나 episode_index 매칭이 필요하지만, 여기서는 전체 비디오 폴더 스캔
    ...
    if os.path.exists(video_dir):
        ...
        # 간단한 로직: 'data' 폴더 내의 parquet 파일 감지
